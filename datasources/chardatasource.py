...
class CharDatasource:
    ...
    def __init__(self, path, seq_len, *args):
        ...
        """Initialize vocabularies for char rnn.
        In our case we want to train only characters and \n (end of word) token
        ...
        self._char2ix = {None:0}
        self._char2ix.update({ch: i+1 for i, ch in enumerate(sorted(chars))})
        self._ix2char = {0:None}
        self._ix2char.update({i+1: ch for i, ch in enumerate(sorted(chars))})
        ...
        while True:
            try:
                ...
                self._tokens.append([self._char2ix[None]] + [self._char2ix[ch] for ch in s])
        ...
        between only most-probable character index and target (see sampling in inference)
        """
