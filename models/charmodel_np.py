...
class CharModelRNN:
    ...
    def __init__(self, clip_ratio, params, seq_len):
        ...
        """Define tuning params. Import updated weights
        (or zero weights).
        """
    ...
    def forward_rnn(self, X, Y, hidden_back):
        ...
        for t in range(self._seq_len):
            ...
            # averaging loss per sequence element
            loss -= np.sum(np.log(output[t].T[range(len(X)), Y[:, t]])) / len(X)

        # saving collected dicts of hidden and output layers
        cache = (output, hidden)
    ...
    def rnn_backward(self, X, Y, cache):
        ...
        """
        Ideas for calculating RNN backprop are mainly taken from
